...
스크립트는 모두 **POSIX sh** (`#!/bin/sh`) 로 짜여 있다. 실제로 검증된 환경:

| 셸 | 결과 |
|---|---|
| bash 5+ | ✅ |
| bash 3.2 (macOS 기본) | ✅ |
| dash (Debian/Ubuntu 기본 sh) | ✅ |
| busybox ash (Alpine 기본 sh) | ✅ |
| zsh sh-mode | ✅ |

bashism (`[[ ]]`, `mapfile`, `local`, `<( )`, `set -o pipefail` 등)은 사용
안 함. zsh user 가 `scripts/up.sh go` 로 실행하면 OS 가 shebang 의 `/bin/sh`
를 호출하므로 zsh 상호작용 셸이어도 스크립트 실행은 sh 컨텍스트.

## 트러블슈팅

- `❌ docker not found in PATH` — Docker Desktop 또는 Docker Engine 설치/실행 확인
- `unknown target` — 첫 인자는 `go` / `all` / `py` 중 하나
- Windows에서 `/bin/sh` 가 없을 때 — Git Bash 또는 WSL 에서 실행 (Git Bash 의 sh 가 `/usr/bin/sh` 로 alias 되어 있음)
